...
public class MergeTest extends BaseMergeTest {
	...
	private static final String QUERY_PATIENT = "select * from patient where patient_id in (select person_id "
	        + "from person where person_id != 1 and uuid != '" + Constants.PHANTOM_UUID + "')";
	...
	@Test
	@Sql({ "classpath:users.sql", "classpath:patient.sql" })
